...
const AppLockSettings = () => {
  ...
  return (
    <section className="rounded-[22px] border border-[#0a0a0a14] bg-white p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          className={`relative mt-1 h-7 w-12 shrink-0 rounded-full transition ${
            config.enabled ? 'bg-[#0a0a0a]' : 'bg-[#c4c4c4]'
          }`}
        >
          <span
            className={`absolute top-1 h-5 w-5 rounded-full bg-[#ffffff] transition-all ${
      ...
      <p className="mt-3 text-xs uppercase tracking-[0.15em] text-[#9a9a9a]">{statusLine}</p>
      {status && <p className="mt-2 text-xs text-[#6b6b6b]">{status}</p>}
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {active && passkeySupported && !hasPasskey && (
            <button
              ...
              disabled={busy}
              className="rounded-full border border-[#0a0a0a] px-4 py-2 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-50"
            >
              Add passkey unlock
            </button>
          )}
      ...
      {setupOpen && (
        <div className="mt-4 rounded-xl border border-[#0a0a0a1f] bg-[#efefec] p-4">
          ...
          <input
            ...
            className="mt-3 w-full rounded-full border border-[#0a0a0a33] bg-white px-4 py-2 text-sm tracking-[0.2em]"
          />
          <input
            type="password"
            inputMode="numeric"
            ...
            className="mt-2 w-full rounded-full border border-[#0a0a0a33] bg-white px-4 py-2 text-sm tracking-[0.2em]"
          />
          {error && <p className="mt-2 text-xs text-[#7f1d1d]">{error}</p>}
          <div className="mt-3 flex gap-2">
            <button
              type="button"
              onClick={() => void saveSetup()}
              disabled={busy}
              className="rounded-full border border-[#0a0a0a] bg-[#0a0a0a] px-4 py-2 text-xs font-semibold text-[#ffffff] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {busy ? 'Saving…' : 'Save'}
            </button>
            <button
              ...
              onClick={closeSetup}
              disabled={busy}
              className="rounded-full border border-[#0a0a0a] px-4 py-2 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-60"
            >
              Cancel
            </button>
          </div>
