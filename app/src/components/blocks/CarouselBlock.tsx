    ...
    <div className="mt-3">
      ...
      >
        {block.cards.map((card, i) => (
          <div
            key={i}
            className="w-[75vw] max-w-[280px] shrink-0 snap-start last:snap-end rounded-2xl border border-ink-fade bg-surface p-4"
          >
      ...
      {block.cards.length > 1 && (
        <div className="mt-1 flex justify-center gap-1.5">
          {block.cards.map((_, i) => (
            <div
              key={i}
              className={`h-1.5 w-1.5 rounded-full ${i === active ? 'bg-ink' : 'bg-ink-fade'}`}
            />
          ))}
        </div>
      )}
