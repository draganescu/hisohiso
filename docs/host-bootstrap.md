# Host bootstrap — provisioning a hisohiso app host
...
| ------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `scripts/bootstrap-host.sh`     | Once per app host (fresh or migrating)              | Installs fail2ban, ufw, unattended-upgrades. Configures auto-reboot. Creates a non-root sudo user with SSH keys.   |
| `scripts/lockdown-sshd.sh`      | Once per app host, AFTER verifying the new user     | Disables root SSH, restricts SSH to the new user only, turns off X11 + TCP forwarding.                             |
...
## Existing app host (migrating from root-only)
...
# Stage the keys (same as fresh-app-host case)
cat > /tmp/deploy-keys <<'EOF'
...
Then close your old root session. Optionally, on the app host:
...
- **Provider firewall.** If your host has a firewall outside the VM, configure it for 80, 443, 443/udp, and SSH. That layer does not fight UFW + Docker iptables rules, and is cleaner than trying to thread UFW through Docker's DOCKER-USER chain.
- **TLS certificates.** Caddy handles them automatically (Let's Encrypt) once the container is up and the domain points at the app host.
- **Backups / snapshots.** Out of scope; use provider snapshots or `borg`/`restic` to a separate host.
...
`lockdown-sshd.sh` is also idempotent in effect (re-running it just rewrites the same drop-in), but you only really need to run it once per app host.
